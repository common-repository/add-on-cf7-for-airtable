		...
		<fieldset>
			...
			<?php if ( strpos( $api_key, 'key' ) !== 0 ) { ?>
				<div class="notice inline notice-error">
					<p>
					<?php
					echo esc_html( __( 'Please save your settings before mapping your fields.', 'add-on-cf7-for-airtable' ) );
					?>
						</p>
				</div>
				<?php
			...
			if ( $table_selected ) {
				...
				?>
				<div id="wpc-wpcf7-airtable-fields-table" class="
				<?php
				echo ! $enabled ? ' is-disabled' : '';
				?>
																	">
					<h3>
					<?php
					echo esc_html( __( 'Table fields', 'add-on-cf7-for-airtable' ) );
					?>
						</h3>
						<?php if ( strpos( $api_key, 'key' ) === 0 ) { ?>
					<p>
							<?php
							echo esc_html( __( 'Enter an Airtable field name for each Contact Form 7 field to map them.', 'add-on-cf7-for-airtable' ) );
							?>
						<br />
							<?php
							echo esc_html( __( 'Be careful, field names should be exactly the same (case sensitive) or the form won\'t be saved.', 'add-on-cf7-for-airtable' ) );
							?>
					</p><?php } ?>
					...
					if ( $first_field && $first_field['column_type_was_empty'] ) {
						...
						<p><strong>
						<?php
						echo esc_html( __( 'Starting from the 1.1.0 version, you can now specify the Airtable field type in the mapping below, please double check all types are fine.', 'add-on-cf7-for-airtable' ) );
						?>
									</strong></p>
						<?php
					}
					...
					<table class="widefat fixed striped" style="max-width: 700px;">
						<thead>
							<tr>
								<th>
								<?php
								echo esc_html( __( 'Contact Form 7 field', 'add-on-cf7-for-airtable' ) );
								?>
									</th>
								<th>
								<?php
								echo esc_html( __( 'Airtable field name', 'add-on-cf7-for-airtable' ) );
								?>
									</th>
								<th>
								<?php
								echo esc_html( __( 'Airtable field type', 'add-on-cf7-for-airtable' ) );
								?>
									</th>
							</tr>
						</thead>
						<tbody>
							...
							foreach ( $contact_form->scan_form_tags() as $tag ) {
								...
								<tr>
									...
									if ( isset( $mapped_tags[ $tag_name ] ) && in_array( $mapped_tags[ $tag_name ]['airtable_field_name'], $available_columns_name, true ) ) {
									...
									?>
										...
										if ( ! empty( $mapped_error_desc ) ) {
											...
											?>
											<div class="wpc-wpcf7-airtable-error">
											<?php
											echo esc_html( $mapped_error_desc );
											?>
																					</div>
											<?php
										}
