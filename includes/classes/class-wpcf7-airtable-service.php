...
class WPCF7_Airtable_Service extends \WPCF7_Service {
	...
	/**
	 * Display a notice to inform users that API keys will be deprecated.
	 *
	 * @return void
	 */
	...
	public function display_setup() {
		...
		<form method="post" action="<?php echo esc_url( $this->menu_page_url( 'action=setup' ) ); ?>">
			...
			echo wp_kses(
				sprintf(
					'<p>%s</p>',
					__( 'Access tokens should at least be given the <code>data.records:write</code> and <code>schema.bases:read</code> scopes.', 'add-on-cf7-for-airtable' )
				),
				array(
					'code' => array(),
					'p'    => array(),
